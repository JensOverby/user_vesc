...
void commands_send_turbine_conf(COMM_PACKET_ID packet_id, turbine_configuration *turbine_conf) {
	chMtxLock(&send_buffer_mutex);
	send_buffer_global[0] = packet_id;
	int32_t len = turbine_conf_serialize(send_buffer_global + 1, turbine_conf);
...
void commands_process_packet(unsigned char *data, unsigned int len,
	...
	case COMM_SET_CUSTOM_CONFIG: {
		turbine_configuration *turbineconf = turbine_conf_alloc();
		*turbineconf = *turbine_conf_get();

		if (turbine_conf_deserialize(data, turbineconf)) {
			//utils_truncate_number(&turbineconf->l_current_max_scale , 0.0, 1.0);
			//utils_truncate_number(&turbineconf->l_current_min_scale , 0.0, 1.0);
			//mcconf->lo_current_max = mcconf->l_current_max * mcconf->l_current_max_scale;

			turbine_conf_store(turbineconf);
			turbine_conf_set(turbineconf);
			chThdSleepMilliseconds(200);
			...
			commands_printf("Warning: Could not set turbineconf due to wrong signature");
		}

		turbine_conf_free(turbineconf);
	} break;
	...
	case COMM_GET_CUSTOM_CONFIG_DEFAULT: {
		...
		if (packet_id == COMM_GET_CUSTOM_CONFIG) {
			*turbineconf = *turbine_conf_get();
			...
			turbine_conf_defaults(turbineconf);
		}

		commands_send_turbine_conf(packet_id, turbineconf);
		turbine_conf_free(turbineconf);
	} break;
